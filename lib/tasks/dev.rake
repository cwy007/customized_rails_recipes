namespace :dev do
  ...
  task :fake_event_and_registrations => :environment do
    event = Event.create!( :status => "public", :name => "全栈营 Meetup", :description => Faker::Lorem.paragraph, :friendly_id => "fullstack-meetup")
    ...
    1000.times do |i|
      event.registrations.create!( :status => ["pending", "confirmed"].sample, # NOTE Array.sample
